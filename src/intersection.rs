...
pub trait Intersection<U: Copy>: Copy {
    ...
    fn intersect(self, other: U) -> Option<Self::Output>;
}
...
/// Returns `Some` when the intersection between the values is not empty; `None` if the intersection is empty.
#[inline]
pub fn intersect<T: Intersection<U>, U: Copy>(value: T, other: U) -> Option<T::Output> {
    value.intersect(other)
}
